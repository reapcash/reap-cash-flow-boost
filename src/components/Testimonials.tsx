          ...
          <div className="bg-card border border-border rounded-3xl p-8 lg:p-12 relative">
            ...
            <div className="flex items-center justify-center gap-4 mt-8">
              ...
              <div className="flex gap-2">
                {testimonials.map((_, index) => <button key={index} className={`w-2 h-2 rounded-full transition-all ${index === currentIndex ? "bg-primary w-8" : "bg-border"}`} onClick={() => setCurrentIndex(index)} aria-label={`Go to testimonial ${index + 1}`} />)}
              </div>
